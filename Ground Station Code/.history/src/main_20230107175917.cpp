...
void createDataLoggingFile()
{
    ...
    // Read the current file number from the EEPROM
    int fileNumber = EEPROM.read(0);

    // Increment the file number by 1
    fileNumber++;

    // Write the updated file number back to the EEPROM
    EEPROM.write(0, fileNumber);
    EEPROM.commit();

    // Create the file name by concatenating the file number with a prefix
    String fileName = "file_" + String(fileNumber) + ".txt";

    // Open a new file on the SD card with the generated file name and the WRITE mode
...
Note : Be sure to replace the fileName variable with your desired file name format, and adjust the EEPROM address as needed.Also, be sure to initialize the SdFat library and the SD card before using it.
}
