...
{ // max message length = 17 characters
    ...
    if (connected)
    {
        ...
        messages.drawString("Connected", 4, 6, 1);
    }
    else
    {
        messages.setTextColor(TFT_RED);
        messages.drawString("LoRa Timed Out", 4, 6, 1);
    }
    messages.unloadFont();
...
void loop()
{
    ...
    if (packetSize)
    {
        ...
        {
            Serial.print("opening data file failed");
        }
    ...
    if (currentMillis - lastRssiCheck > TIMEOUT)
    {
