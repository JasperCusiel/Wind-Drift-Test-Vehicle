...
void createDataLoggingFile()
{
    ...
    if (!sd.exists(newFileName))
    {
        ...
        if (newFile.open(newFileName, O_CREAT | O_WRITE))
        {
            ...
            newFile.print("Time UTC (H:M:S),Latitude (DD\xB0),Longitude (DD\xB0), Altitude (m), GPS Ground Speed (m/s), GPS Track Over Ground (deg\xB0), Primary Temperature (C\xB0), Humidity (RH%), Battery Percentage (%)");
            newFile.println();
            newFile.close();
        }
        else
