...
#include <LoRa.h>
#include <OneButton.h>
#include <Adafruit_TinyUSB_Core.h> // include the TinyUSB Core library
#include <Adafruit_TinyUSB_Msc.h>  // include the TinyUSB MSC library
...
// }
// define the msc_device_read function:
uint32_t msc_device_read(uint8_t lun, uint8_t *buffer, uint32_t sector, uint8_t count)
{
    // read data from the SD card:
    if (!sd.read(sector, buffer, count))
    {
...
// define the msc_device_write function:
uint32_t msc_device_write(uint8_t lun, const uint8_t *buffer, uint32_t sector, uint8_t count)
{
    // write data to the SD card:
    if (!sd.write(sector, buffer, count))
    {
...
void loop()
{
    ...
    { // button is pressed
        // start the MSC device:
        if (!usb_msc.isRunning())
        {
            usb_msc.begin();
        }
    }
