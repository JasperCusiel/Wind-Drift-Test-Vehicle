...
void loop1()
{
    // Check if a LoRa message is available
    ...
    if (packetSize)
    {
        ...
        // Parse the string and extract the sensor values
        ...
        while (index != -1)
        {
            ...
            index = inputString.indexOf(DELIMITER, lastIndex);

            // Extract the value between the last delimiter and the current one
            String valueString = inputString.substring(lastIndex, index);

            // Convert the value to an integer
            int value = valueString.toInt();

            // Store the value in the appropriate sensor variable
            if (lastIndex == 0)
            {
                sensor1 = value;
            }
            ...
            // Update the last index to search from the next character
            lastIndex = index + 1;
        }

        int start = message.indexOf("[");
        int end = message.indexOf("]");
        String sensorData = message.substring(start + 1, end);
        ...
        int latitude = sensorData.substring(0, sensorData.indexOf(",")).toInt();
        int longitude = sensorData.substring(sensorData.indexOf(",") + 1).toInt();
        int altitude = sensorData.substring()
                           tft.startWrite();
        drawLayout(100, -60, testSend, testReceive, testHeading, 13.5, 20, 100);
        // use a switch statement to check the value of the mode variable:
        switch (mode)
        ...
        // if the mode value is 2, do something else:
        case 1:
            drawGpsPage(latitude, longitude);
            break;
