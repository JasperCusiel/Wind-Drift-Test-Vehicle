...
int messageIndex = 0;
bool newMessage = false; // flag to indicate if a new message has been received
...
void onReceive(int packetSize)
{
    ...
    {
        message[messageIndex] = (char)LoRa.read();
        messageIndex++;
    }
    message[messageIndex] = '\0'; // Null terminate the char array
    messageIndex = 0;             // Reset the message index for the next message

    // Print the received message
    Serial.println(message);
    float latitude, longitude, altitute, temp, humidity, heading, bat, groundSpeed;
    String time;
    sscanf(message, "%s, %f, %f, %f, %f, %f, %f, %f, %f", &time, &latitude, &altitute, &temp, &humidity, &heading, &bat, &groundSpeed);
    testLat = latitude;
    testLong = longitude;
...
void loop1()
{
    ...
    if (packetSize)
    {
        ...
        message[packetSize] = '\0'; // Null terminate the char array

        // Extract sensor data from the message
        int latitude, longitude;

        sscanf(message, "%d,%d", &latitude, &longitude);
