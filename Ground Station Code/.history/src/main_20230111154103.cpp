...
void loop()
{
    ...
    if (packetSize)
    {
        ...
        sscanf(message, "%d:%d:%d.%d,%d,%f,%f,%d,%f,%f,%f,%f,%f", &hour, &min, &sec, &millisec, &gpsFixVal, &latitude, &longitude, &altitute, &groundSpeed, &trackOverGround, &temp, &humidity, &bat);

        testLat = latitude;
        testLong = longitude;
        testAltitude = (altitute * 3.28084); // convert to feet
        testTemp = temp;
