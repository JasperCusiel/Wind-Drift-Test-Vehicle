...
void setup1()
{
    ...
    // Enable the CRC for received packets
    LoRa.enableCrc();
...
void loop1()
{
    ...
    if (packetSize)
    {
        ...
        if (LoRa.crc())
        {
            // CRC is valid, process the packet
            processPacket(packet);
        }
        else
        {
            // CRC is invalid, discard the packet
        ...
        int sensor1 = sensorData.substring(0, sensorData.indexOf(",")).toInt();
        int sensor2 = sensorData.substring(sensorData.indexOf(",") + 1).toInt();
