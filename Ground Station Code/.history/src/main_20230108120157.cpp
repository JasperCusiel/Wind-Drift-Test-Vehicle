...
void loop()
{
    ...
    {
        tft.startWrite();
        drawLayout(batteryPercentage, -60, testSend, testReceive, testHeading, testGroundSpeed, testTemp, testHumdity);
...
void loop1()
{
    ...
    if (packetSize)
    {
        ...
        float latitude, longitude, temp, humidity, heading, bat, groundSpeed;
        int altitute;
        int hour, min, sec;
        sscanf(message, "%d:%d:%d,%f,%f,%d,%f,%f,%f,%f,%f", &hour, &min, &sec, &latitude, &longitude, &altitute, &temp, &humidity, &heading, &bat, &groundSpeed);
        testLat = latitude;
        testLong = longitude;
