...
void drawStatusBar(float percentage, int rssi, bool transmitting, bool receiving, bool isConnected, int fixType)
{
    ...
    {
        statusBar.pushImage(28, 5, 10, 10, gpsFix);
    }
    else
    {
        statusBar.pushImage(28, 5, 10, 10, gpsNoFix);
    }
...
void drawLayout(int batteryPercentage, int rssi, bool transmitting, bool receiving, int heading, float velocity, int temp, int humidity, bool isConnected, int gpsFixType)
{
    /**
     ...
     */
    drawStatusBar(batteryPercentage, rssi, transmitting, receiving, isConnected, gpsFixType);
...
void loop()
{
    ...
    if (packetSize)
    {
        ...
        float groundSpeed, trackOverGround, temp, humidity, bat, altitute, latitude, longitude;
        int hour, min, sec, millisec, gnssFixType;

        sscanf(message, "%d:%d:%d.%d,%d,%.6f,%.6f,%.0f,%.1f,%.1f,%.1f,%.1f,%.1f", &hour, &min, &sec, &millisec, &gnssFixType, &latitude, &longitude, &altitute, &groundSpeed, &trackOverGround, &temp, &humidity, &bat);
    ...
    downButton.tick();
    tft.startWrite();
    drawLayout(batteryPercentage, rssi, testSend, testReceive, vehicleHeading, testGroundSpeed, testTemp, testHumdity, currentlyConnected, testFixType);
