...
{ // max message length = 17 characters
  // if ((connected != lastStatus) && (count < 14))
  // {
  //     messages.scroll(0, 1);
  //     count++;
  // }
  ...
  //         messages.drawString("LoRa Timed Out", 0, 0, 1);
  //     }
  //     messages.unloadFont();
  // }
...
void loop()
{
    ...
    //     else
    //     {
    //         Serial.print("failed to open data logging file");
    //     }
    ...
    // }
    tft.startWrite();
    drawLayout(batteryPercentage, rssi, testSend, testReceive, vehicleHeading, testGroundSpeed, testTemp, testHumdity, true);
