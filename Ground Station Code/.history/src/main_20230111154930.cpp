...
void loop()
{
    ...
    if (packetSize)
    {
        ...
        float groundSpeed, trackOverGround, temp, humidity, bat, latitude, longitude;
        ;
        int hour, min, sec, millisec, gpsFixVal, altitude;

        sscanf(message, "%d:%d:%d.%d,%d,%f,%f,%d,%f,%f,%f,%f,%f", &hour, &min, &sec, &millisec, &gpsFixVal, &latitude, &longitude, &altitude, &groundSpeed, &trackOverGround, &temp, &humidity, &bat);

        testLat = latitude;
        testLong = longitude;
        testAltitude = (altitude * 3.28084); // convert to feet
        testTemp = temp;
