...
void loop()
{
    ...
    if (packetSize)
    {
        ...
        float groundSpeed, trackOverGround, temp, humidity, bat, altituted, latitude, longitude;
        ;
        int hour, min, sec, millisec, gpsFixVal;

        sscanf(message, "%d:%d:%d.%d,%d,%f,%f,%d,%f,%f,%f,%f,%f", &hour, &min, &sec, &millisec, &gpsFixVal, &latitude, &longitude, &altituted, &groundSpeed, &trackOverGround, &temp, &humidity, &bat);

        testLat = latitude;
        testLong = longitude;
        testAltitude = (altituted * 3.28084); // convert to feet
        testTemp = temp;
