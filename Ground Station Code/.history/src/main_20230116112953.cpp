...
{
    /**
     * @brief This function is called when the up button is clicked and changes the mode to display the next page
     */
    if (mode == 1)
    {
        mode = 0;
...
{
    /**
     * @brief This function is called when the down button is clicked and changes the mode to display the next page
     */
    if (mode == 0)
    {
        mode = 1;
...
void setup()
{
    rp2040.idleOtherCore(); // Idle other core such that the setup dont interfere with checking if the up button is pressed on boot
    ...
    tft.setRotation(1);
    tft.setSwapBytes(true);
    tft.pushImage(0, 0, 160, 128, splashImg); // Draw splash screen boot image
    ...
    fileCreated = createDataLoggingFile(); // Create data logging file

    if (!fileCreated) // Check if creating file was succesfull
    {
        Serial.println("datalogging file failed");
        return;
    }
    tft.fillScreen(TFT_BLACK); // Draw blank screen to hide splash screen
}

void setup1()
{
    LoRa.setPins(csPin, resetPin, irqPin);
    if (!LoRa.begin(915E6)) // Start LoRa module at 915Mhz
    {
...
void loop()
{
    int packetSize = LoRa.parsePacket(); // Check if new LoRa packet is available
    if (packetSize)
    {
        for (int i = 0; i < packetSize; i++) // Read the packet and store it in the character array
        {
            message[i] = (char)LoRa.read();
        }

        message[packetSize] = '\0'; // Add a null terminator to the end of the message

        rssi = LoRa.packetRssi(); // Get packet RSSI
        currentlyConnected = true;
        lastRssiCheck = millis(); // Update last time RSSI was checked (used to determine if LoRa is still connected)
        noInterrupts();           // Disable interupts while writing to SD so button callbacks don't interupt SD write
        dataFile.println(message);
        if (!dataFile.sync())
        {
            Serial.println("Error writing LoRa data to SD");
            return;
        }
        interrupts();            // Enable interupts to allow buttons to work
        Serial.println(message); // Print received message to serial
        // Varibles to store data pulled from LoRa string
        float groundSpeed, trackOverGround, temp, humidity, bat, latitude, longitude, altitude;
        int hour, min, sec, millisec, gpsFixVal;

        // Split message into seperate varibles
        ...
        testLat = latitude;
        testLong = longitude;
        testAltitude = int((altitude * 3.281)); // Convert to altitude in meters to feet
        testTemp = temp;
        testHumdity = humidity;
        vehicleHeading = trackOverGround;
        batteryPercentage = bat;
        testGroundSpeed = (groundSpeed * 1.943844); // Convert speed in m/s to knots
        testGpsFixType = gpsFixVal;
        testReceive = true;
    }
    if (millis() - lastRssiCheck > TIMEOUT) // If no new RSSI value has been received for TIMEOUT seconds, set connection status to false (disconnected)
    {
        Serial.println("LoRa timedout - no signal received");
    ...
    tft.startWrite();
    drawLayout(batteryPercentage, rssi, testSend, testReceive, vehicleHeading, testGroundSpeed, testTemp, testHumdity, currentlyConnected, testGpsFixType); // Draw variales onto screen
    switch (mode)
    {
    case 0:
    ...
    case 1:
        if ((testGpsFixType == 2) || (testGpsFixType == 3)) // Only update the lat and long if the received packet GPS fix type was 2D or 3D
        {
            passLat = testLat;
            passLong = testLong;
        }
