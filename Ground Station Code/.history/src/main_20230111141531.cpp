...
void loop()
{
    ...
    if (packetSize)
    {
        ...
        sscanf(message, "%d:%d:%d.%d,%d,%f,%f,%.0f,%.1f,%.1f,%.1f,%.1f,%.1f", &hour, &min, &sec, &millisec, &gnssFixType, &latitude, &longitude, &altitute, &groundSpeed, &trackOverGround, &temp, &humidity, &bat);
