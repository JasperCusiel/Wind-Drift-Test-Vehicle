...
void loop1()
{
    ...
    if (packetSize)
    {
        ...
        testLat = sensorData.substring(0, sensorData.indexOf(",")).toFloat();
        testLong = sensorData.substring(sensorData.indexOf(",") + 1).toFloat();
        ...
        if (dataFile.open(filename, FILE_WRITE))
        {
            dataFile.println(sensor1 + "," + sensor2);
            dataFile.close();
        }
        else
        {
            tft.println("Error opening sensor_data.txt");
        }
    }
}