...
// Declare the circular buffer and the write and display threads
...
void loop1()
{
    ...
    if (packetSize)
    {
        ...
        // Search for the start and end of the message
        ...
        {
            String data = packet.substring(startIndex + 1, endIndex);
            int commaIndex = data.indexOf(",");
            testLat = data.substring(0, commaIndex).toFloat();
            data = data.substring(commaIndex + 1);
            commaIndex = data.indexOf(",");
            testLong = data.substring(0, commaIndex).toFloat();
            batteryPercentage = data.substring(commaIndex + 1).toFloat();

            // Add the sensor data to the buffer
            String output = String(testLat) + "," + String(testLong) + "," + String(batteryPercentage);
            for (int i = 0; i < output.length(); i++)
            {
                buffer[end] = output[i];
                end = (end + 1) % BUFFER_SIZE;

                // If the buffer is full, reset the pointers
                if (end == start)
                {
                    start = 0;
                    end = 0;
