...
void logGPSData(UBX_NAV_PVT_data_t *ubxDataStruct)
{
  ...
  sprintf(dataBuffer, "%d:%d:%d.%d,%d,%.4f,%.4f,%.2f,%.2f,%.2f,%d,%d,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%d", hour, min, sec, millisecs, timeValid, gpsLatitude, gpsLongitude, gpsAltitude, gpsGroundSpeed, gpsHeading, satelitesInView, fixType, externalTemp, externalHumidity, altimeterTemp, altimeterAltitude, lipoStateOfCharge, lipoDischargeRate, timestamp);
  int time2 = millis();
  lastTime = time2;
...
void setup1()
{
  ...
  dataFile.open(newFileName, FILE_WRITE);
