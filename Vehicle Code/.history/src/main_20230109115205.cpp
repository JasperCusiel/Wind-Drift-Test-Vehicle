...
// Time (in milliseconds) between blinks at the start of the sequence
const int INITIAL_BLINK_DELAY = 1000;

// Time (in milliseconds) between blinks at the end of the sequence
const int FINAL_BLINK_DELAY = 100;

// Time (in milliseconds) for the entire sequence to run
const int SEQUENCE_DURATION = 6000;

// Timestamp of the last time the LED was turned on or off
...
void blinkPowerDownLed()
{
  // Calculate the current time into the sequence
  unsigned long currentTime = millis() % SEQUENCE_DURATION;

  // Calculate the current delay between blinks
  unsigned long currentDelay = map(currentTime, 0, SEQUENCE_DURATION, INITIAL_BLINK_DELAY, FINAL_BLINK_DELAY);

  // If it's time to update the LED state, do so
  if (millis() - lastUpdateTime >= currentDelay)
  {
    // Toggle the LED state
    int ledState = digitalRead(LED_GREEN) == LOW ? HIGH : LOW;
...
void setup1()
{
  ...
  powerButton.attachDuringLongPress(powerButtonPressed);
  powerButton.attachLongPressStart(startSequence);
  powerButton.attachLongPressStop(stopSequence);
}
