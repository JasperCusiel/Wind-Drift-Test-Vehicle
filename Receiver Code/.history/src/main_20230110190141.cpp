...
void loop1()
{
  if (mode == 1)
  {
    ...
    int currentPPSState = digitalRead(pin); // read the current state of the pin
    if (currentPPSState != previousState)
    { // if the state has changed
      if (currentPPSState == HIGH)
      { // if the pin is now high
        // do something
      }
      else
      { // if the pin is now low
        // do something else
      }
    }
    previousState = currentPPSState; // update the previous state with the current state
    if (ppsLedCount == 10)
    {
      digitalWrite(LED_BLUE, HIGH);
      ppsLedCount = 0;
