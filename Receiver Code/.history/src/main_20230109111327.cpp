...
void logGPSData()
{
  ...
  if (dataFile.open(newFileName, FILE_WRITE))
  {
    ...
    sprintf(dataBuffer, "%d:%d:%d,%d,%.2f,%.2f,%.2f,%.2f,%.2f,%d,%d,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%d", hour, min, sec, timeValid, gpsLatitude, gpsLongitude, gpsAltitude, gpsGroundSpeed, gpsHeading, satelitesInView, fixType, externalTemp, externalHumidity, altimeterAltitude, lipoStateOfCharge, lipoDischargeRate, timestamp);
